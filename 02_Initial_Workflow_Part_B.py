...
# MAGIC
# MAGIC **NOTE** Please note that linking these records with the images is a time-consuming task so that we've elected to provide the supporting code in script. 

# COMMAND ----------

# DBTITLE 1,Retrieve Matches
matches = (
  spark
    .read
    .format('delta')
...
# get image paths for limited matches
image_paths = (
  matches_limited
    .selectExpr("replace(image_path, '/dbfs/', 'dbfs:/') as image_path")
    .distinct()
    .toPandas()['image_path']
    .to_list()
  )

# retrieve those images to dataframe
images = (
  spark
    .read
    .format('image')
    .option('ignoreCorruptFiles','true')
    .load(path=image_paths)
    .selectExpr(
...
display(
  matches_limited_with_images
    .select('z_cluster','z_minScore','z_maxScore','item_id','item_name','image')
    .orderBy('z_cluster')    
    )
...
# MAGIC
# MAGIC Also, we can look at the number of records within each cluster.  Most clusters will consist of 1 or 2 records.  That said there will be other clusters with abnormally large numbers of entries that desere a bit more scrutiny:
